...
static int get_pixel_value(unsigned int x, unsigned int y) {
  int color;
  int k;
...
void paint(void) {
  ...
  while (fgets(buf, 512, stdin)) {
    ...
    if (strspn(t, "0123456789") == strlen(t))
      i = strtoul(t, NULL, 10);
    else if (1 == inet_pton(AF_INET6, t, &a6)) {
      i = (a6.s6_addr[0] << 24) | (a6.s6_addr[1] << 16) | (a6.s6_addr[2] << 8) |
          a6.s6_addr[3];
...
void watermark(gdImagePtr i) {
  int color = gdImageColorAllocateAlpha(i, 127, 127, 127, 63);
  gdImageStringUp(i, gdFontGetSmall(), gdImageSX(i) - 20, 220,
...
static void annotate(gdImagePtr i) {
  if (shadings)
